...
(defun ctg-win--do-delete (name)
  (when-let ((m (ht-get ctg-win--window-configs name)))
    (ht-remove! ctg-win--window-configs name)))

(defun ctg-win/delete-window-state ()
  "Delete a previously saved window state.

The given window state will be delete from the list of available states."
  (interactive)
  (ivy-read "Select the window configuration: "
            (ht-map (lambda (k v) (format "%s" k)) ctg-win--window-configs)
            :action #'ctg-win--do-delete))
